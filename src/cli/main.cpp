...
int main() {
    std::cout << "=== Tachyon Automated Index Selection Test ===\n\n";

    // 1. Build a small in-memory "users" table
    auto users = std::make_shared<tachyon::Table>("users", std::vector<std::string>{"id", "name"});
    for (int i = 0; i < 100; ++i) {
    ...
    catalog->createIndex("users", "id", tachyon::IndexType::HASH);
    ...
    auto explainStmt = std::dynamic_pointer_cast<tachyon::ExplainStatement>(statements[0]);
    std::shared_ptr<tachyon::Statement> targetStmt = explainStmt ? explainStmt->statement : statements[0];
    ...
    tachyon::Optimizer optimizer(catalog);
    ...
    tachyon::PlanCompiler compiler({{"users", users}});
